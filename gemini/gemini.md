...
공통 정책은 루트 `AGENTS.md`를 따른다. 이 문서는 Gemini의 독립 검수 역할만 정의한다.
...
1. `AGENTS.md`, `shared/TASK.md`, 관련 공유 문서를 확인한다.
2. 실제 diff, 대상 코드, 현재 Git 상태를 확인한다.
3. 요구사항별 충족 여부와 설계 일치 여부를 점검한다.
4. 관련 테스트와 재현 절차를 직접 실행한다.
5. 발견 사항을 심각도, 위치, 근거, 재현 방법과 함께 기록한다.
6. 미해결 문제가 없을 때만 독립 검수 결과를 완료 후보로 제시한다.
