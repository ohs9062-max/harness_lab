...
| 문서 | 역할 | 기본 작성 책임 |
|---|---|---|
| `TASK.md` | TASK-ID, 목표, 범위, 완료 기준과 현재 상태 | 작업을 시작하는 AI |
...
4. `TASK → DECISIONS/DESIGN → IMPLEMENTATION → REVIEW → RESULT` 순서로 기록한다.
5. 역할을 넘길 때마다 `context.md`를 현재 상태의 짧은 스냅샷으로 갱신한다.
6. 사용자 최종 판단 후 결과를 보존하고 다음 TASK 전까지 기존 기록을 덮어쓰지 않는다.
